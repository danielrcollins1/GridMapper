...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  switch (message) {
    case WM_PAINT:      MyPaintWindow(hWnd); break;
    case WM_SIZE:       SetScrollRange(hWnd, false); break;
    case WM_HSCROLL:    HorzScrollHandler(hWnd, wParam); break;
    case WM_VSCROLL:    VertScrollHandler(hWnd, wParam); break;
    case WM_KEYDOWN:    MyKeyHandler(hWnd, wParam); break;
...
bool CommandProc (HWND hWnd, int cmdId) {
  ...
  switch (cmdId) {
    case IDM_OPEN:      OpenMap(hWnd); break;
    case IDM_SAVE:      SaveMap(hWnd); break;
    case IDM_SAVE_AS:   SaveMapAs(hWnd); break;
    case IDM_COPY:      CopyMap(hWnd); break;
    case IDM_PRINT:     PrintMap(hWnd); break;
    case IDM_EXIT:      DestroyWindow(hWnd); break;
    case IDM_FILL_MAP:  ClearMap(hWnd, false); break;
    case IDM_CLEAR_MAP: ClearMap(hWnd, true); break;
    case IDM_NEW:
      DialogBox(hInst, (LPCTSTR)IDD_NEWMAP, hWnd, (DLGPROC)NewDialog); break;
    case IDM_SET_GRID_SIZE:
      DialogBox(hInst, (LPCTSTR)IDD_SETGRIDSIZE, hWnd, (DLGPROC)GridSizeDialog); break;
    case IDM_ABOUT:
...
void SetScrollRange (HWND hWnd, bool zeroPos) {
  ...
  SCROLLINFO infoHorz = {sizeof(SCROLLINFO), SIF_ALL, 0, hMax, rw.right, hPos, 0};
  ...
  SCROLLINFO infoVert = {sizeof(SCROLLINFO), SIF_ALL, 0, vMax, rw.bottom, vPos, 0};
...
void HorzScrollHandler(HWND hWnd, WPARAM wParam) {
  ...
  switch (LOWORD(wParam)) {
    case SB_LINEUP: info.nPos -= GetGridSize(); break;
    case SB_LINEDOWN: info.nPos += GetGridSize(); break;
    case SB_PAGEUP: info.nPos -= info.nPage; break;
    case SB_PAGEDOWN: info.nPos += info.nPage; break;
    case SB_THUMBPOSITION: info.nPos = HIWORD(wParam); break;
    case SB_THUMBTRACK: info.nPos = HIWORD(wParam); break;
...
void MyKeyHandler(HWND hWnd, WPARAM wParam) {
  switch (wParam) {
    ...
    case VK_HOME:   SendMessage(hWnd, WM_HSCROLL, SB_TOP, 0); 
                    SendMessage(hWnd, WM_VSCROLL, SB_TOP, 0); break;
    case VK_END:    SendMessage(hWnd, WM_HSCROLL, SB_BOTTOM, 0); 
...
void FloorSelect (HWND hWnd, int xPos, int yPos) {
  ...
  switch (selectedFeature) {
	...
	case IDM_FLOOR_NEWALL:     newFeature = FLOOR_NEWALL; break;
	case IDM_FLOOR_NWWALL:     newFeature = FLOOR_NWWALL; break;
