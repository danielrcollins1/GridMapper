...
void GridMap::setCellFloor(int x, int y, int floor) 
  { grid[x][y].floor = floor; changed = true; };
void GridMap::setCellNWall(int x, int y, int wall) 
  { grid[x][y].nwall = wall; changed = true; };
void GridMap::setCellWWall(int x, int y, int wall) 
  { grid[x][y].wwall = wall; changed = true; };
void GridMap::setCellObject(int x, int y, int object) 
  { grid[x][y].object = object; changed = true; };
void GridMap::setFilename(char *name) 
  { strncpy(filename, name, GRID_FILENAME_MAX); };

// Empty cell & any adjacent walls
void GridMap::clearCell(int x, int y) {
  grid[x][y].floor = FLOOR_FILL;
  grid[x][y].wwall = WALL_CLEAR;
  grid[x][y].nwall = WALL_CLEAR;
  if (x < width-1)    
    grid[x+1][y].wwall = WALL_CLEAR;
  if (y < height-1)   
    grid[x][y+1].nwall = WALL_CLEAR;
...
void GridMap::paint(HDC hDC) {
  ...
  // Draw extreme border (esp. bottom-right)
  SelectObject(hDC, ThinGrayPen);
  SelectObject(hDC, GetStockObject(NULL_BRUSH));
  Rectangle(hDC, 0, 0, getWidthPixels()+1, getHeightPixels()+1);
}
